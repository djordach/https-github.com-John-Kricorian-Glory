...
import Foundation
 ...
 public class XMLCustomParser: NSObject {
    
    func getElementsByTagName(data: Data, tagName: String) -> [XMLNode]? {
        let dom = MicroDOM(data: data)
        let tree = dom.parse()
        guard let nodes = tree?.getElementsByTagName(tagName) else { return nil }
        return nodes
    }
    
    func parseData(data: Data) -> XMLNode? {
        let dom = MicroDOM(data: data)
        let tree = dom.parse()
        return tree
    }
